...
export class ProjectBetaSource implements MangaSource {
  id = "project-beta";
  name = "project-beta";
  baseUrl = "https://manhwadesu.shop";
  private client = new HttpClient(this.baseUrl);

  async getPopular(page: number): Promise<MangaPageResult> {
    throw new Error("project-beta: Sedang dalam tahap pengembangan adapter.");
  }
