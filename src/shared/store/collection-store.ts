  ...
  persist(
    (set, get) => {
      const syncCloud = () => {
        setTimeout(() => {
          const state = get();
          pushCustomCollections(state.collections, state.membershipsByManga).catch(() => {});
        }, 0);
      };
      ...
      return {
        ...
        syncWithCloud: (cloudCollections: Collection[], cloudMemberships: Record<MangaKey, string[]>) => {
          set((state) => {
            ...
            state.collections.forEach(c => collectionsMap.set(c.id, c));
            cloudCollections.forEach(c => {
              const local = collectionsMap.get(c.id);
              if (!local || new Date(c.updatedAt).getTime() > new Date(local.updatedAt).getTime()) {
                collectionsMap.set(c.id, c);
              }
            });

            // Merge memberships
            const mergedMemberships: Record<MangaKey, string[]> = { ...state.membershipsByManga };
            Object.entries(cloudMemberships || {}).forEach(([mangaKey, colIds]) => {
              const existing = mergedMemberships[mangaKey] || [];
              const combined = Array.from(new Set([...existing, ...colIds]));
              mergedMemberships[mangaKey] = combined;
            });

            const mergedCollections = Array.from(collectionsMap.values()).sort((a, b) => a.sortOrder - b.sortOrder);
            return {
              collections: mergedCollections,
              membershipsByManga: mergedMemberships,
            };
          });
        },
      };
