...
export const YomirraSurface = React.forwardRef<HTMLDivElement, YomirraSurfaceProps>(
  ({ className, variant = "base", ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn( "transition-colors", { "bg-surface-base text-text-primary": variant === "base", "bg-surface-raised text-text-primary": variant === "raised", "bg-surface-overlay text-text-primary shadow-sm border-border-subtle": variant === "elevated", "bg-surface-overlay text-text-primary -glass shadow-default": variant === "floating", "bg-surface-base/80 backdrop-blur-xl supports-[backdrop-filter]:bg-surface-base/60 shadow-sm": variant === "glass", "bg-surface-muted text-text-primary": variant === "muted",},
          className
        )}
        {...props}
      />
